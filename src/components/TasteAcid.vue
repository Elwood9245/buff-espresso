...
import { ACID_OPTIONS, ACID_UNACCEPTABLE_KEYS } from '../types'
...
const customChips = computed(() =>
  model.value.selected.filter(w => !(ACID_OPTIONS as readonly string[]).includes(w))
)
...
watch(
  ...
  (selected) => {
    const hasUnacceptable = ACID_UNACCEPTABLE_KEYS.some(k => selected.includes(k))
...
function clearUnacceptable() {
  if (model.value.unacceptable) {
    ...
    for (const key of ACID_UNACCEPTABLE_KEYS) {
...
<template>
  <div class="space-y-4">
    <h3 class="text-lg font-bold text-stone-800">酸 · 味觉评估</h3>

    <div>
      <p class="text-sm text-stone-500 mb-2">选择描述词（可多选）</p>
      <div class="flex flex-wrap gap-2">
        <button
          v-for="opt in ACID_OPTIONS"
          :key="opt"
          @click="toggle(opt)"
          :class="[
            'px-4 py-2 rounded-full text-sm font-medium transition-all min-h-[44px]',
            isSelected(opt)
              ? (ACID_UNACCEPTABLE_KEYS as readonly string[]).includes(opt)
    ...
    <div class="flex items-center gap-3 py-2">
      ...
      <span class="text-sm text-stone-600">
        不能接受 <span class="text-stone-400">（刺激、尖锐）</span>
      </span>
    </div>
